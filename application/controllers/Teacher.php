...
class Teacher extends CI_Controller
{
    ...
    function subject($param1 = '', $param2 = '' , $param3 = '')
    {
        ...
        {   
            $i=1;
            foreach ($this->input->post('lesson') as $key) {

                $data['week']=$i;
                $data['id_subject']=$param2;
                $data['name']=$key;
                echo $this->db->insert('lesson' , $data);
                $i++;
            }
            // echo $this->session->userdata('login_user_id');
            // echo "$param2";
            // print_r($this->input->post('lesson'));
  ...
  //       $this->load->view('backend/index', $page_data);
    ...
    function homework_add($subject='', $lesson) 
    {   
        $page_data['subject_id']=$subject_id;
    ...
    function homework_bridge_detail($lesson='')
    {
        // echo "string";
        $homework_code = $this->db->get_where('homework',array('lesson_id'=>$lesson))->row()->homework_code;
        redirect(base_url() . 'index.php?teacher/homeworkroom/details/'.$homework_code, 'refresh');
    }
    ...
    function class_list()
    {
        $query = $this->db->get_where('subject' , array('subject_id' => $this->session->userdata('teacher_login')))->result_array();
        $page_data['class_id']   = $param1;
        $page_data['subjects']   = $query;
        $page_data['page_name']  = 'coursess';
        $page_data['page_title'] = get_phrase('Manage-Subjects');
        $this->load->view('backend/index', $page_data);
    }

    function lesson_list($param1='')
    {
        $query = $this->db->get_where('lesson' , array('id_subject' => $param1))->result_array();
        $page_data['lesson']=$query;
        $page_data['param1']=$param1;//subject_id
        $page_data['page_name'] = 'lesson';
        $page_data['page_title'] = get_phrase('Lesson');
        $this->load->view('backend/index', $page_data);
        // print_r($query);
    }

    function upload_file_lesson($param1='', $param2='', $param3='')
    {
        if ($param1=='create') {
            // param3 = subject id
            // param2 = lesson id

            $id_teacher = $this->db->get_where('subject' , array('subject_id' => $param3))->row()->teacher_id;
            // echo $id_teacher;
            $name_subject = $this->db->get_where('subject' , array('subject_id' => $param3))->row()->name;
            $name_teacher =$this->db->get_where('teacher' , array('teacher_id' => $id_teacher))->row()->name;
            $name_lesson = $this->db->get_where('lesson' , array('id_lesson' => $param2))->row()->name;
            // echo 'uploads/lesson/'.$name_teacher.'/'.$name_lesson;
            if (!file_exists('uploads/lesson/'.$name_teacher.'/'.$name_subject.'/'.$name_lesson)) {
                mkdir('uploads/lesson/'.$name_teacher.'/'.$name_subject.'/'.$name_lesson, 0777, true);
            }
            // print_r($this->input->post('file_name'));

            foreach ($_FILES["file_name"]["tmp_name"] as $key => $value) {
                // echo $keyprin
                move_uploaded_file($value, 'uploads/lesson/'.$name_teacher.'/'.$name_subject.'/'.$name_lesson .'/'. basename($_FILES["file_name"]["name"][$key]));
                $data['id_lesson'] = $param2;
                $data['url']=base_url().'uploads/lesson/'.$name_teacher.'/'.$name_subject.'/'.$name_lesson .'/'. basename($_FILES["file_name"]["name"][$key]);
                echo $data['name_file'] = $this->input->post('name')[$key];
                $data['date'] = $this->input->post('tanggal')[$key];
                echo $this->db->insert('file_lesson' , $data);
            ...
            // }
            redirect(base_url() . 'index.php?teacher/lesson_list/'.$param3, 'refresh');
        }
        else{
            $page_data['page_name'] = 'upload_file_lesson';
            $page_data['subject_id']=$this->db->get_where('lesson' , array('id_lesson' => $param1))->row()->id_subject;
            $page_data['materi']=$this->db->get_where('lesson' , array('id_lesson' => $param1))->row()->name;
            $page_data['page_title'] = get_phrase('Upload-Files');
            $page_data['id_lesson']=$param1;
            // print_r($this->db->get_where('lesson' , array('id_lesson' => $param1))->num_rows());
            $this->load->view('backend/index', $page_data);
        }
    }

    function get_class_lesson($subject_id) 
    {
        $lesson = $this->db->get_where('lesson' , array(
            'id_subject' => $subject_id
        ))->result_array();
        echo '<option value="">Select </option>';
        foreach ($lesson as $row) {
            echo '<option value="' . $row['id_lesson'] . '">' . $row['name'] . '</option>';
        }
    }

    function delete_file_lesson($param1='', $param2='')
    {
        // $param1=id_file
        // $param2=id_subject
        
        $this->db->where('id_file_lesson', $param1);
        if($this->db->delete('file_lesson')){
            redirect(base_url() . 'index.php?teacher/lesson_list/'.$param2, 'refresh');
        }
    }
}