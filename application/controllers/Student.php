...
class Student extends CI_Controller
{
    ...
    function homework_upload($param1 = '')
    {
        // echo "string";
        // homeworl_student/teacher/matkul/materi/mahasiswa/

        $homework       =   $this->db->get_where('homework' , array('homework_code' => $param1))->row();
        print_r($_FILES);
        $teacher_name = $this->db->get_where('teacher' , array('teacher_id' => $homework->uploader_id))->row()->name;

        $lesson_name    =   $this->db->get_where('lesson', array('id_lesson' => $homework->lesson_id))->row()->name;
        
        $subject_id = $this->db->get_where('lesson', array('id_lesson' => $homework->lesson_id))->row()->id_subject;

        $subject_name = $this->db->get_where('subject', array('subject_id' => $subject_id))->row()->name;
        // echo $subject_name;
        // print_r($this->db->get_where('subject', array('subject_id' => $subject_id))->row());

        $student_name = $this->db->get_where('student', array('student_id' => $this->session->userdata('student_id')))->row()->name;

        echo 'uploads/homework_student/'.$teacher_name.'/'.$subject_name.'/'.$lesson_name.'/'.$student_name;
        // print_r($student_name);
        // print_r($teacher_name);
        // $class_name     =   $this->db->get_where('class', array('class_id' => $homework->class_id))->row()->name;
        // $data['file_name'] = $_FILES["file_name"]["name"];
        // $this->db->where('homework_id', $param1);
        // $this->db->update('homework', $data);

        if (!file_exists('uploads/homework_student/'.$teacher_name.'/'.$subject_name.'/'.$lesson_name.'/'.$student_name))
            mkdir('uploads/homework_student/'.$teacher_name.'/'.$subject_name.'/'.$lesson_name.'/'.$student_name, 0777, true);

        move_uploaded_file($_FILES["file_name"]["tmp_name"], 'uploads/homework_student/'.$teacher_name.'/'.$subject_name.'/'.$lesson_name.'/'.$student_name.'/'. basename($_FILES["file_name"]["name"]));
        $data['homework_id'] = $homework->homework_id;
        $data['student_id'] = $this->session->userdata('student_id');
        $data['url'] = base_url().'uploads/homework_student/'.$teacher_name.'/'.$subject_name.'/'.$lesson_name.'/'.$student_name.'/'. basename($_FILES["file_name"]["name"]);
        $data['name']=basename($_FILES["file_name"]["name"]);
        echo $this->db->insert('homework_student' , $data);
        redirect(base_url() . 'index.php?student/homeworkroom/details/'.$param1, 'refresh');
    }
   ...
   function invoice($param1 = '', $param2 = '', $param3 = '')
    ...
    function homeworkroom($param1 = '' , $param2 = '')
    {
        ...
        if ($param1 == 'file') 
        {
            ...
            // $page_data['id_file_homework']=$param3;
        }  
    ...
    function reload_homeworkroom_details($homework_code = '', $status='') {
        if ($status == 'sukses') {
